...
DATA_STORE_CHAT_ID = -1003929535070  # data সেভ channel
DATA_PIN_MSG_ID = None  # pinned message ID
group_night_msg_id = None  # রাতের মেসেজ ID সেভ
group_reminder_msg_id = None  # ওয়ার্নিং মেসেজ ID সেভ
allowed_users = set([OWNER_ID])
user_target_groups = {}  # {user_id: {group_id: group_title}} — প্রতি user এর আলাদা গ্রুপ
user_names = {}  # {user_id: name} — ইউজারের নাম সেভ

BD_TZ = timezone(timedelta(hours=6))

data = {}  # {user_id: { broadcast_msg, broadcast_photo, is_running, interval, next_broadcast_time, last_msg_ids }}
...
# ============================================
# Data Save — channel এ pin করো
# ============================================
async def save_data(bot):
    global DATA_PIN_MSG_ID
    payload = {
        "user_target_groups": {
            str(uid): {str(gid): title for gid, title in groups.items()}
            for uid, groups in user_target_groups.items()
        },
        "allowed_users": list(allowed_users),
        "user_names": {str(uid): name for uid, name in user_names.items()},
        "user_data": {
            str(uid): {
                "broadcast_msg": udata.get("broadcast_msg"),
                "broadcast_photo": udata.get("broadcast_photo"),
                "interval": udata.get("interval"),
                "is_running": udata.get("is_running", False),
            }
    ...
    try:
        if DATA_PIN_MSG_ID:
            await bot.edit_message_text(
                chat_id=DATA_STORE_CHAT_ID,
                message_id=DATA_PIN_MSG_ID,
                text=text
            )
        else:
            ...
            DATA_PIN_MSG_ID = msg.message_id
...
# ============================================
# Data Load — pinned message থেকে লোড করো
# ============================================
async def load_data(bot, app=None):
    global DATA_PIN_MSG_ID, user_target_groups, allowed_users, user_names
    try:
        chat = await bot.get_chat(DATA_STORE_CHAT_ID)
        if chat.pinned_message:
            DATA_PIN_MSG_ID = chat.pinned_message.message_id
            text = chat.pinned_message.text
            text = text.replace("📦 BOT DATA:\n", "")
            payload = json.loads(text)
            user_target_groups = {
                int(uid): {int(gid): title for gid, title in groups.items()}
                for uid, groups in payload.get("user_target_groups", {}).items()
            }
            allowed_users = set(payload.get("allowed_users", [OWNER_ID]))
            user_names = {int(uid): name for uid, name in payload.get("user_names", {}).items()}
            for uid_str, udata in payload.get("user_data", {}).items():
                uid = int(uid_str)
                get_user_data(uid)["broadcast_msg"] = udata.get("broadcast_msg")
                get_user_data(uid)["broadcast_photo"] = udata.get("broadcast_photo")
                get_user_data(uid)["interval"] = udata.get("interval")
                get_user_data(uid)["is_running"] = udata.get("is_running", False)

                # broadcast চালু ছিলে অটো restart করো
                if udata.get("is_running") and udata.get("interval") and app:
                    interval = udata["interval"]
                    app.job_queue.run_repeating(
                        broadcast_job,
                        interval=interval,
                        first=interval,
                        name=f"broadcast_loop_{uid}",
                        data=uid
                    )
                    logging.info(f"✅ broadcast job restarted for user {uid}")

            logging.info("✅ Data loaded from Telegram channel!")
    except Exception as e:
...
async def allow_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != OWNER_ID:
        if is_allowed(update.effective_user.id):
            await update.message.reply_text(
                "⚠️ এই কমান্ডটি আপনার জন্য নয়!\n"
                "🔒 শুধুমাত্র Admin ব্যবহার করতে পারবে।"
            )
        else:
            await access_denied(update)
        return
    ...
    try:
        ...
        try:
            await context.bot.send_message(
                chat_id=new_id,
                text="🎉✅ অভিনন্দন!\n\nআপনাকে বটের অ্যাক্সেস দেওয়া হয়েছে! 🟢\n\nএখন শুরু করতে /start চাপুন 🚀"
            )
        except Exception as e:
            logging.error(f"Allow notify error: {e}")
    except ValueError:
...
async def remove_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            await context.bot.send_message(
                chat_id=rem_id,
                text="🚫❌ আপনাকে ব্যান করা হয়েছে! 🔴\n\n⏳ পরে আবার চেষ্টা করুন 🙏"
            )
        except Exception as e:
            logging.error(f"Remove notify error: {e}")
    except ValueError:
        await update.message.reply_text("❌ সঠিক User ID দিন!")
...
async def set_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"📛 নাম: {title}\n"
        f"🆔 আইডি: {gid}\n"
        f"📡 মোট গ্রুপ: {len(get_user_groups(user_id))} টি\n"
    )
...
async def show_groups(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, (gid, title) in enumerate(my_groups.items(), 1):
        ...
        keyboard.append([
            InlineKeyboardButton(f"❌ {title} বাদ দিন", callback_data=f"delgroup_{user_id}_{gid}")
        ])

    await update.message.reply_text(
        f"  📋 টার্গেট গ্রুপ লিস্ট   \n"
        f"📡 মোট গ্রুপ: {len(my_groups)} টি\n"
        f"{group_lines}"
...
async def setmsg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not get_user_groups(update.effective_user.id):
        await update.message.reply_text(
            "⚠️ আগে গ্রুপ সেট করুন!\n\n"
            "📌 যেভাবে করবেন:\n"
            "1. বটকে গ্রুপে Admin করুন\n"
            "2. সেই গ্রুপে গিয়ে /set দিন\n"
            "3. তারপর /setmsg দিন ✅"
    ...
    if update.message.photo:
        ...
        get_user_data(update.effective_user.id)["broadcast_msg"] = caption
        get_user_data(update.effective_user.id)["broadcast_photo"] = update.message.photo[-1].file_id
    else:
        ...
        get_user_data(update.effective_user.id)["broadcast_msg"] = msg_text
        get_user_data(update.effective_user.id)["broadcast_photo"] = None
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cb.startswith("delgroup_"):
        parts = cb.replace("delgroup_", "").split("_")
        owner_id = int(parts[0])
        gid = int(parts[1])

        # শুধু নিজের গ্রুপ বাদ দিতে পারবে
        if user_id != owner_id:
            await query.answer("🚫 এটা আপনার গ্রুপ না!", show_alert=True)
            return
        ...
        for g_id, g_title in my_groups.items():
            keyboard.append([
                InlineKeyboardButton("❌ বাদ দিন", callback_data=f"delgroup_{owner_id}_{g_id}")
            ])

        await query.edit_message_text(
            f"✅ '{title}' বাদ দেওয়া হয়েছে!\n\n"
            f"  📋 টার্গেট গ্রুপ লিস্ট   \n"
            f"📡 বাকি গ্রুপ: {len(my_groups)} টি\n"
    ...
    # ইন্টারভাল সিলেক্ট
        ...
        udata["interval"] = seconds
        udata["is_running"] = True

        # শুধু এই user এর পুরনো job বন্ধ করো
        old_jobs = context.application.job_queue.get_jobs_by_name(f"broadcast_loop_{user_id}")
        for job in old_jobs:
        ...
        await query.edit_message_text(
            ...
            f"⛔ বন্ধ করতে /stop দিন। অথবা চেক /status"
        )

        await send_broadcast(context, user_id)
        udata["next_broadcast_time"] = datetime.now(timezone.utc).timestamp() + seconds

        context.application.job_queue.run_repeating(
            broadcast_job,
            interval=seconds,
            first=seconds,
            name=f"broadcast_loop_{user_id}",
            data=user_id  # user_id পাঠাচ্ছি job এ
        )
...
async def broadcast_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not udata["is_running"]:
        jobs = context.application.job_queue.get_jobs_by_name(f"broadcast_loop_{user_id}")
        for job in jobs:
            job.schedule_removal()
        return

    await send_broadcast(context, user_id)
    interval = udata.get("interval") or 900
    udata["next_broadcast_time"] = datetime.now(timezone.utc).timestamp() + interval


# ============================================
# send_broadcast — সব user এর সব গ্রুপে পাঠাও
# ============================================
async def send_broadcast(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    udata = get_user_data(user_id)
    msg = udata.get("broadcast_msg")
    photo = udata.get("broadcast_photo")

    if not msg:
        return

    my_groups = get_user_groups(user_id)
    for group_id in list(my_groups.keys()):
        try:
            old_msg_id = udata["last_msg_ids"].get(group_id)
            if old_msg_id:
                try:
            ...
            udata["last_msg_ids"][group_id] = sent.message_id
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data_u = get_user_data(update.effective_user.id)

    if not data_u["is_running"]:
        await update.message.reply_text(
            "⚠️ ব্রডকাস্ট ইতিমধ্যে বন্ধ আছে!\n\n"
            "▶️ চালু করতে /setmsg দিন।"
        )
        return

    data_u["is_running"] = False
    data_u["next_broadcast_time"] = None
    data_u["broadcast_msg"] = None
    data_u["broadcast_photo"] = None
    data_u["interval"] = None

    jobs = context.application.job_queue.get_jobs_by_name(f"broadcast_loop_{update.effective_user.id}")
    for job in jobs:
        job.schedule_removal()
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    interval = udata.get("interval")
    next_time = udata.get("next_broadcast_time")

    countdown_text = "N/A"
    if udata["is_running"] and next_time:
    ...
    msg_preview = udata.get("broadcast_msg") or "সেট হয়নি"
    if len(msg_preview) > 25:
        msg_preview = msg_preview[:25] + "..."

    await update.message.reply_text(
        f"  📊 বর্তমান অবস্থা  \n\n"
        f"🔄 ব্রডকাস্ট: {'✅ চালু' if udata['is_running'] else '🛑 বন্ধ'}\n"
        f"📡 আমার গ্রুপ: {len(my_groups)} টি\n"
        f"💬 মেসেজ: {msg_preview}\n"
...
async def time_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text and "time" in text.strip().lower():
        now_bd = datetime.now(BD_TZ)
        await update.message.reply_text(
            f"🕰️ এখন সময়!!\n\n"
...
async def refer_code_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        reply = await update.message.reply_text(
            ...
            f"`{text}#1`\n\n"
            f"💡 বিশেষ দ্রষ্টব্য:\n"
            f"উপরের কোডের উপর ক্লিক করলে\n"
            f"অটোমেটিক কপি হয়ে যাবে! 👆\n"
            f"তারপর এই গ্রুপে পেস্ট করুন। ✅\n"
            f"কোন স্পেস দিবেন না 😩",
            parse_mode="Markdown"
        )
...
async def user_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    users = [uid for uid in allowed_users if uid != OWNER_ID]

    if not users:
        await update.message.reply_text(
            "👥 অনুমোদিত ইউজার লিস্ট\n\n"
            "⚠️ এখনো কাউকে পারমিশন দেওয়া হয়নি।"
        )
        return
    ...
    for i, uid in enumerate(users, 1):
        ...
        user_lines += f"{i}. 🆔 `{uid}`{name_text}\n"

    await update.message.reply_text(
        f"👥 অনুমোদিত ইউজার লিস্ট\n\n"
...
async def post_init(application):
    await load_data(application.bot, application)

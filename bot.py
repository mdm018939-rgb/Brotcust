...
SUPABASE_KEY = "sb_secret_og8bvg6YD5bC-oOb-oRk-Q_VEYf_DIM"
SUPABASE_TABLE = "bot_data"
DATA_KEY = "main"


async def supabase_request(method, payload=None):
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            if method == "GET":
                full_url = url + f'?key=eq.{DATA_KEY}&select=value'
...
async def save_data(bot):
    payload = {
        ...
        # নতুন per-group broadcast data সেভ
        "group_broadcast_data": {
            str(uid): {
                str(gid): {
                    "broadcast_msg": gdata.get("broadcast_msg"),
                    "broadcast_photo": gdata.get("broadcast_photo"),
                    "broadcast_video": gdata.get("broadcast_video"),
                    "broadcast_media_group": gdata.get("broadcast_media_group"),
                    "interval": gdata.get("interval"),
                    "is_running": gdata.get("is_running", False),
                    "last_msg_id": gdata.get("last_msg_id"),
                    "next_broadcast_time": gdata.get("next_broadcast_time"),
                }
                for gid, gdata in user_groups.items()
            }
            for uid, user_groups in group_broadcast_data.items()
        },
        "user_group_schedule": {
            str(uid): {
                str(gid): {
                    ...
                    "night_msg_id": sdata.get("night_msg_id"),
                }
    ...
    try:
        result = await supabase_request("POST", {
            "key": DATA_KEY,
        ...
        if result is not None:
            ...
            logging.error("❌ Supabase save failed!")
    except Exception as e:
...
async def load_data(bot, app=None):
    global user_target_groups, group_broadcast_data
    try:
        result = await supabase_request("GET")
        if not result:
            logging.warning("⚠️ No data from Supabase.")
            return

        rows = json.loads(result)
        if not rows:
            logging.warning("⚠️ Supabase table empty.")
            return

        payload = json.loads(rows[0]["value"])

        global all_bot_users, user_lang
        all_bot_users = set(payload.get("all_bot_users", []))
        user_lang = {int(uid): lang for uid, lang in payload.get("user_lang", {}).items()}
        ...
        # পুরনো user রাও all_bot_users এ যোগ করো
        all_bot_users.update(user_target_groups.keys())

        # নতুন per-group broadcast data লোড
        raw_gbd = payload.get("group_broadcast_data", {})
        group_broadcast_data = {}
        for uid_str, user_groups in raw_gbd.items():
            uid = int(uid_str)
            group_broadcast_data[uid] = {}
            for gid_str, gdata in user_groups.items():
                gid = int(gid_str)
                saved_next_time = gdata.get("next_broadcast_time")
                group_broadcast_data[uid][gid] = {
                    "broadcast_msg": gdata.get("broadcast_msg"),
                    "broadcast_photo": gdata.get("broadcast_photo"),
                    "broadcast_video": gdata.get("broadcast_video"),
                    "broadcast_media_group": gdata.get("broadcast_media_group"),
                    "interval": gdata.get("interval"),
                    "is_running": gdata.get("is_running", False),
                    "next_broadcast_time": saved_next_time,
                    "last_msg_id": gdata.get("last_msg_id"),
                }

                # broadcast চালু ছিলে অটো restart
                if gdata.get("is_running") and gdata.get("interval") and app:
                    interval = gdata["interval"]
                    # কতক্ষণ বাকি ছিল হিসাব করো
                    now_ts = __import__('datetime').datetime.now(__import__('datetime').timezone.utc).timestamp()
                    if saved_next_time and saved_next_time > now_ts:
                        first = saved_next_time - now_ts  # বাকি সময়
                    else:
                        first = 5  # আগেই পাঠানোর সময় হয়ে গেছে, ৫ সেকেন্ড পরে পাঠাও
                    group_broadcast_data[uid][gid]["next_broadcast_time"] = now_ts + first
                    app.job_queue.run_repeating(
                        broadcast_job,
                        interval=interval,
                        first=first,
                        name=f"broadcast_{uid}_{gid}",
                        data={"user_id": uid, "group_id": gid}
                    )
                    logging.info(f"✅ broadcast job restarted for user {uid} group {gid} (first={first:.0f}s)")

        # শিডিউল লোড
        global user_group_schedule
        raw_schedule = payload.get("user_group_schedule", {})
        user_group_schedule = {
            int(uid): {
                int(gid): {
                    "off_hour": sdata.get("off_hour"),
            ...
            for uid, schedules in raw_schedule.items()
        }

        logging.info("✅ Data loaded from Supabase!")
    except Exception as e:
        ...
        if other_admin_id:
            ...
            if existing_pending:
                ...
                if remaining > 0:
                    ...
                    existing_pending = False
                    # return নেই — নিচে "রিকুয়েস্ট পাঠান" বাটন দেখাবে
...
async def check_missed_schedules(application):
    ...
    if tasks:
        await asyncio.gather(*tasks)
        await save_data(application.bot)
